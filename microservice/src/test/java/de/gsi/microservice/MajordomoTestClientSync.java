...
public class MajordomoTestClientSync {
     ...
     */
    public MajordomoProtocol.MdpMessage send(final String service, final byte[]... msgs) {
     ...
     * @return reply message or NULL if there was no reply
     */
    public MajordomoProtocol.MdpMessage send(final byte[] service, final byte[]... msgs) {
        ZMsg reply = null;
        ...
        while (retriesLeft > 0 && !Thread.currentThread().isInterrupted()) {
            ...
            if (!new MajordomoProtocol.MdpMessage(null, PROT_CLIENT, GET_REQUEST, service, "requestID".getBytes(StandardCharsets.UTF_8), topic, msgs[0], "", rbacToken).send(clientSocket)) {
