...
import static de.gsi.microservice.MajordomoProtocol.Command.GET_REQUEST;
import static de.gsi.microservice.MajordomoProtocol.Command.SUBSCRIBE;
import static de.gsi.microservice.MajordomoProtocol.EMPTY_FRAME;
import static de.gsi.microservice.MajordomoProtocol.MdpSubProtocol.PROT_CLIENT;
...
public class MajordomoTestClientAsync {
     ...
     */
    public MajordomoProtocol.MdpMessage recv() {
        ...
        if (poller.pollin(0)) {
            return MajordomoProtocol.MdpMessage.receive(clientSocket, false);
        }
        return null;
    }
    ...
    public boolean send(final byte[] service, final byte[]... msgs) {
        ...
        return new MajordomoProtocol.MdpMessage(null, PROT_CLIENT, GET_REQUEST, service, "requestID".getBytes(StandardCharsets.UTF_8), URI.create(topic), msgs[0], "", rbacToken).send(clientSocket);
    }
    ...
    public boolean subscribe(final byte[] service, final byte[]... rbacToken) {
        ...
        return new MajordomoProtocol.MdpMessage(null, PROT_CLIENT, SUBSCRIBE, service, "requestID".getBytes(StandardCharsets.UTF_8), URI.create(topic), EMPTY_FRAME, "", rbacTokenByte).send(clientSocket);
    }
