...
import static de.gsi.microservice.MajordomoProtocol.Command.DISCONNECT;
import static de.gsi.microservice.MajordomoProtocol.Command.FINAL;
import static de.gsi.microservice.MajordomoProtocol.Command.W_HEARTBEAT;
import static de.gsi.microservice.MajordomoProtocol.EMPTY_FRAME;
import static de.gsi.microservice.MajordomoProtocol.EMPTY_URI;
import static de.gsi.microservice.MajordomoProtocol.MdpMessage;
import static de.gsi.microservice.MajordomoProtocol.MdpMessage.receive;
import static de.gsi.microservice.MajordomoProtocol.MdpSubProtocol;
import static de.gsi.microservice.MajordomoProtocol.MdpSubProtocol.PROT_WORKER;
...
public class MajordomoBroker extends Thread {
    ...
    public void addInternalService(final MajordomoWorker worker) {
    ...
    public Socket bindPub(String endpoint) {
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.atDebug().addArgument(endpoint).log("Majordomo broker/0.1 publication is active at '{}'");
        }
        return pubSocket;
    }
            ...
            while (run.get() && !Thread.currentThread().isInterrupted() && items.poll(HEARTBEAT_INTERVAL) != -1) {
                ...
                while (run.get() && !Thread.currentThread().isInterrupted() && receivedMsg) {
                    ...
                    if (subMsg != null) {
                        ...
                        final byte[] subMessage = !subMsg.isEmpty() ? subMsg.getFirst().getData() : new byte[] { 1 }; //TODO: refactor into an enum/class representation
                        final String subType = subMessage[0] == 1 ? "subscribe" : "unsubscribe";
                        final String subTopic = subMessage.length > 1 ? new String(subMessage, 1, subMessage.length - 1, StandardCharsets.UTF_8) : "*";
    ...
    protected void deleteWorker(Worker worker, boolean disconnect) {
        assert (worker != null);
        if (disconnect) {
            new MdpMessage(worker.address, PROT_WORKER, DISCONNECT, worker.service.nameBytes, EMPTY_FRAME, URI.create(worker.service.name), EMPTY_FRAME, "", RBAC).send(worker.socket);
        }
    ...
    protected void processClients() {
        ...
        clients.forEach((name, client) -> {
            ...
            if (service == null) {
                ...
                new MdpMessage(clientMessage.senderID, client.protocol, FINAL, clientMessage.serviceNameBytes, clientMessage.clientRequestID,
                        URI.create("mdp/UnknownService"), "501".getBytes(StandardCharsets.UTF_8), "", RBAC)
                        .send(client.socket);
                return;
            }
    ...
    protected /*synchronized*/ void purgeClients() {
        ...
        for (String clientName : clients.keySet()) { // NOSONAR NOPMD copy because we are going to remove keys
    ...
    protected void registerDefaultServices(final RbacRole<?>[] rbacRoles) {
        ...
        mmiService.registerHandler(ctx -> {
            final String serviceName = (ctx.req.data != null) ? new String(ctx.req.data, StandardCharsets.UTF_8) : "";
            final String returnCode;
            if (!serviceName.isBlank()) {
                returnCode = services.containsKey(serviceName) ? "200" : "400";
            } else {
                returnCode = services.keySet().stream().sorted().collect(Collectors.joining(","));
            }
        ...
        mmiDns.registerHandler(ctx -> {
            final String returnCode = routerSockets.stream().map(s -> s.toLowerCase(Locale.UK).replace("tcp", "mdp")).collect(Collectors.joining(","));
            ctx.rep.data = returnCode.getBytes(StandardCharsets.UTF_8);
        });
        ...
        for (int i = 0; i < nServiceThreads; i++) {
            ...
            echoService.registerHandler(ctx -> ctx.rep.data = ctx.req.data); //  output = input : echo service is complex :-)
            addInternalService(echoService);
        }

        // Hello World service
        MajordomoWorker httpDemoService = new MajordomoWorker(ctx, "mmi.hello", rbacRoles);
        httpDemoService.registerHandler(ctx -> ctx.rep.data = "Hello World!".getBytes(StandardCharsets.UTF_8));
    ...
    protected Service requireService(final String serviceName, final byte[] serviceNameBytes) {
        ...
        return services.computeIfAbsent(serviceName, s -> new Service(serviceName, serviceNameBytes, null));
    }
    ...
    protected /*synchronized*/ void workerWaiting(Worker worker) {
        ...
        //TODO: evaluate addLast vs. push (addFirst) - latter should be more beneficial w.r.t. CPU context switches (reuses the same thread/context frequently
